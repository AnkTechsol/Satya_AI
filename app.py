            ...
            for alert in alerts:
                ...
                <div style="border-left: 5px solid {border_color}; background: {bg_color}; padding: 0.8rem; border-radius: 4px; margin-bottom: 0.8rem; color: var(--text-primary);">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.3rem;">
                        <span style="font-weight: 800; color: {text_color}; font-size: 0.85rem;">{html.escape(str(severity))} ALERT</span>
    ...
    with col_b:
        ...
        if not goal_alerts:
            ...
            for alert in reversed(goal_alerts[-10:]):
                ...
                <div style="border-left: 5px solid {border_color}; background: {bg_color}; padding: 0.8rem; border-radius: 4px; margin-bottom: 0.8rem; color: var(--text-primary);">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.3rem;">
                        <span style="font-weight: 800; color: {text_color}; font-size: 0.85rem;">GOAL DRIFT {html.escape(str(action))}</span>
                        <span style="font-size: 0.75rem; color: var(--text-muted);">{html.escape(str(alert.get('timestamp', ''))[:19].replace('T', ' '))}</span>
                    </div>
                    <div style="font-size: 0.85rem; font-weight: 600; margin-bottom: 0.2rem;">
                        Drift Score: {html.escape(str(alert.get('score', '')))}
                    </div>
