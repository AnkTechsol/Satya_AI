...
if page == "Dashboard":
    ...
    with col_left:
        ...
        if sorted_tasks:
            # ⚡ Bolt Optimization: Batching Streamlit markdown calls
            # Rendering markdown individually inside loops creates heavy
            # Streamlit communication overhead and UI thread blocking.
            # We batch the HTML strings and render them with a single call.
            batched_recent_tasks_html = []
            for task in sorted_tasks:
                priority = task.get("priority", "Medium")
                batched_recent_tasks_html.append(f"""
                ...
                """)
            st.markdown("".join(batched_recent_tasks_html), unsafe_allow_html=True)
        else:
            st.markdown("""
