...
## Repository Status
- **Last Analytics Run:** 2026-05-16T14:51:49.376328+00:00Z
- **Open Issues:** Unknown
...
- **Export Adapter Framework (OTLP/Langfuse/LangSmith)**: Added on May 2026. A small, modular adapter system that enables Satya to export traces/events to Langfuse, LangSmith, or OTLP. It keeps the core lightweight while letting enterprises reuse existing investments.
  - *Validation command:* `pytest tests/test_langfuse_adapter.py` and `pytest tests/test_langsmith_adapter.py`
  - *Runbook:* Initialize `SatyaClient(adapters=[LangfuseAdapter(public_key="...", secret_key="...")])` or `SatyaClient(adapters=[LangSmithAdapter(api_key="...", project_name="...")])`.
