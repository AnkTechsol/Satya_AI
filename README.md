...
## Repository Status
- **Last Analytics Run:** 2026-07-01T14:51:38.220201+00:00Z
- **Open Issues:** Unknown
...
- **Export Adapter Framework (OTLP/Console/LangSmith)** (Added 2024-03)
  ...
  - Runbook: Pass a list of instantiated adapters to `satya.init(adapters=[OTLPAdapter(), LangSmithAdapter("my-api-key")])`.
...
- **Export Adapter Framework (OTLP/Langfuse/LangSmith)**: Added on April 2026. A small, modular adapter system that enables Satya to export traces/events to Langfuse, LangSmith, or OTLP. It keeps the core lightweight while letting enterprises reuse existing investments. Includes prompt trace routing for token-level LLM observability.
  - *Validation command:* `pytest tests/test_langsmith_adapter.py`
  - *Runbook:* Initialize `SatyaClient(adapters=[LangSmithAdapter(api_key="...", project_name="my-project")])`.
