...
## Repository Status
- **Last Analytics Run:** 2026-04-04T14:45:07.135069+00:00
...
- **Durable Append-Only Audit Store (SQLite Fallback)** (Added 2026-04)
  - Replaces fragile flat-file audit logs with a durable SQLite database `audit.db` to satisfy enterprise compliance audits while maintaining zero-config portability. It supports seamless rollback to JSONL if the DB is unavailable.
  - Runbook: Runs automatically. No manual setup needed.
  - Validation: Verify `satya_data/events/audit.db` is populated during agent runs.
