...
## Repository Status
- **Last Analytics Run:** 2026-07-10T15:06:04.164742+00:00Z
- **Open Issues:** Unknown
...
- **Export Adapter Framework (OTLP/Console/LangSmith/File)** (Added 2024-03, updated 2026-06)
  - Enables routing Satya's flat-file telemetry traces into enterprise observability stacks without breaking zero-DB architecture. LangSmith adapter enables token/prompt observability.
  - Runbook: Pass a list of instantiated adapters to `satya.init(adapters=[OTLPAdapter(), LangSmithAdapter(), FileAdapter("traces.jsonl")])`. LangSmith requires `LANGSMITH_API_KEY`.
  - Validation: `PYTHONPATH=. pytest tests/test_adapters.py tests/test_adapters_extra.py`
