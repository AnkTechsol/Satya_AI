    ...
    def _send():
        for url in urls_to_notify:
            ...
            try:
                ...
                # Use custom adapter to enforce safe IP resolution while maintaining SNI
                session.mount(f"{parsed.scheme}://", HostHeaderSSLAdapter(safe_ip))

                # Now request the original URL securely
