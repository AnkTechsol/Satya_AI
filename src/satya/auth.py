...
from .core import storage, db, db_postgres
...
def append_audit_event(agent_id: str, task_id: str, trace_id: str, action: str, details: str, prev_hmac: str = "") -> str:
    ...
    # Opt-in Postgres+S3 Durable Audit Store
    if os.environ.get("SATYA_POSTGRES_DB_URI"):
        try:
            db_postgres.append_event_to_db(event)
        except Exception as e:
            print(f"Failed to append to Postgres Audit Store: {e}")
